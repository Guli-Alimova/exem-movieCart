...
// birinchi ish
const movieEl = document.getElementById('cinema');
...
function createmovieDiv(filmobject) {
    ...
    <img src="img/${filmobject.poster}" class="movie__poster">
    <div class="movie__info">
        <h2 class="movie__title">${filmobject.title}</h2>
        <h3 class="movie__genre">${filmobject.genres}</h3>
        <button class="movie__btn" type="button" data-id="${filmobject.id}">Add to watchlist</button>
    </div>
...
if (movieEl) appendMovies();

// categoryga ajratish

// uchinchi ish

function filterfilmCat(category) {
    let result = movies.filter((el) => {
        return el.genres.includes(category)
    });
    return result;
...
const movieBottonEl = document.getElementsByClassName('sidebar')[0];
if (movieBottonEl) {
    movieBottonEl.addEventListener('click', function (event) {
        if (event.target.className == 'genre__item');
        let selectCat = event.target.innerText;
        let selectFilm;
        if (selectCat !== 'All') {
            selectFilm = filterfilmCat(selectCat);
        } else {
            selectFilm = movies
        }

        movieEl.innerText = ''
        appendselectFilm(selectFilm)
...
function appendselectFilm(el) {
    el.forEach((el) => {
        movieEl.appendChild(createmovieDiv(el))
    });
}
...
function searchfilm(searchText) {
    let resultfilm = movies.filter((el) => {
...
searchInput.addEventListener('keyup', function (event) {
    let searchmovies = searchfilm(searchInput.value);

    movieEl.innerHTML = ' ';
    appendselectFilm(searchmovies);
})


//  watchlistga qoshish 8- ish
let myCart = cartItemsFromLocalStorage()
// let myCart = []

function addMovieTowatch(id) {
...
// addMovieTowatch(565028);
// addMovieTowatch(459151);


if (movieEl) {
    movieEl.addEventListener('click', function (event) {
        // console.log(event.target.className == "movie__btn");
        if (event.target.className == "movie__btn") {
            // console.log(+event.target.getAttribute('data-id'));
            let idcurrentmovie = +event.target.getAttribute('data-id');
            addMovieTowatch(idcurrentmovie);
...
function cartItemsToLocalStorage() {
    localStorage.setItem('watchMovieLetter', JSON.stringify(myCart));
}

function cartItemsFromLocalStorage() {
    let result = JSON.parse(localStorage.getItem('watchMovieLetter'));
...
const movieList = document.getElementById('watchItems');
...
function showWatchItems() {
    movieList.innerHTML = '';
    let watchMovieLetterArr = cartItemsFromLocalStorage();
  
    watchMovieLetterArr.forEach((el) => {
        console.log(el);
        let film = movies.filter((items) => {
            return items.id == el;
        });
        let element = createWatchMovie(film[0]);
        movieList.appendChild(element);
    });
}

// remove watchlist 10-ish
...
if(movieList){
    movieList.addEventListener('click', function (event){
        if(event.target.className == 'movie__btn'){
            ...
            watchMovieLetterArr.splice(watchMovieLetterArr.indexOf(+elementId), 1);
            console.log(watchMovieLetterArr);

            localStorage.setItem('watchMovieLetter', JSON.stringify(watchMovieLetterArr));
            showWatchItems()
        }
    })
}
